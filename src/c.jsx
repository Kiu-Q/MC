...
const App = () => {
    ...
    useEffect(() => {
        ...
        tokens.forEach(token => {
            // Strict check: word must contain ONLY A, B, C, or D.
            if (/^[A-D]+$/.test(token)) {
                ...
                img.onload = () => {
                    ...
                    pageRegions.forEach(region => {
                        ...
                        if (region.type === 'id') {
                                ...
                                for (let py = cy + paddingY; py < cy + ch - paddingY; py++) {
                                    for (let px = cx + paddingX; px < cx + cw - paddingX; px++) {
                                        if (px < canvas.width && py < canvas.height) {
                                            ...
                                            totalIntensity += (255 - val);
                                            totalPixelCount++;
                                        }
                                    }
                                }
                                const fillRatio = totalPixelCount > 0 ? (totalIntensity / totalPixelCount) / 255 : 0;
                            ...
                            const MIN_ID_THRESHOLD = 0.05; // Lowered to 5% to catch very light marks
                            ...
                            rowLayouts.forEach((rowConfig, rowIndex) => {
                                ...
                                for (let c = 0; c < region.cols; c++) {
                                    ...
                                    const cw = Math.floor(cellW);
                                    const ch = Math.floor(rowConfig.h);
                                    // Use reduced padding (20%) for answers too to catch edge marks
                                    const paddingX = cw * 0.20; 
                                    const paddingY = ch * 0.20;
                                    let totalIntensity = 0;
                                    let totalPixelCount = 0;
                                    for (let py = cy + paddingY; py < cy + ch - paddingY; py++) {
                                        for (let px = cx + paddingX; px < cx + cw - paddingX; px++) {
                                ...
                                // Revised Answer Logic (Candidate Style)
                                const MIN_ANSWER_THRESHOLD = 0.12; // 12% to filter faint noise
                                // Filter candidates >= 12%
                                const answerCandidates = colScores.filter(s => s.fillRatio >= MIN_ANSWER_THRESHOLD);

                                if (answerCandidates.length === 0) {
                                    label = 'BLANK';
                                } else if (answerCandidates.length === 1) {
                                    selectedIndex = answerCandidates[0].index;
                                    label = region.labels[selectedIndex];
                                } else {
                                    // Multiple candidates > 12%
                                    const winner = answerCandidates[0];
                                    const runnerUp = answerCandidates[1];
                                    
                                    // Check dominance (> 5% difference)
                                    // Reduced from 10% to 5% to allow small margins to win
                                    if ((winner.fillRatio - runnerUp.fillRatio) > 0.05) {
    ...
    const exportExcel = () => {
        ...
        const statsData = [
            ...
            ["Max Score", { t:'n', f: `MAX(${scoreRange})` }],
            ["Min Score", { t:'n', f: `MIN(${scoreRange})` }],
            ["Mean Score", { t:'n', f: `AVERAGE(${scoreRange})`, z:'0.00' }],
            ["Median Score", { t:'n', f: `MEDIAN(${scoreRange})` }],
            ["Standard Deviation", { t:'n', f: `STDEV.S(${scoreRange})`, z:'0.00' }],
            ["Passing Score (50%)", { t:'n', f: `B2*0.5` }], // B2 is Full Mark
            ["No. of Passes (50%)", { t:'n', f: `COUNTIF(${scoreRange},">="&B8)` }], // B8 is Passing Score 50%
            ["Passing Rate (50%)", { t:'n', f: `B9/${studentCount}`, z:'0.0%' }],
            ["Passing Score (40%)", { t:'n', f: `B2*0.4` }], 
            ["No. of Passes (40%)", { t:'n', f: `COUNTIF(${scoreRange},">="&B11)` }], // B11 is Passing Score 40%
            ["Passing Rate (40%)", { t:'n', f: `B12/${studentCount}`, z:'0.0%' }]
        ];
    ...
    return (
        <div className="flex h-screen bg-slate-950 text-slate-200 font-sans overflow-hidden">
            ...
            <div className="w-80 bg-slate-900 border-r border-slate-700 flex flex-col shadow-xl z-10">
                <div className="p-4 border-b border-slate-700 bg-slate-900 text-white flex justify-between items-start">
                    <div>
                    ...
                    <a 
                        href="https://github.com/your-repo/blob/main/Documentation/README.md" // Placeholder
                        target="_blank" 
                    ...
                    <div className="flex items-center gap-4 text-sm text-slate-400 overflow-hidden">
                        ...
                        {pages.length > 1 && (
                            <div className="flex items-center bg-slate-800 rounded-lg p-1 gap-2 ml-4">
                                <button onClick={() => { setCurrentPageIndex(p => Math.max(0, p - 1)); setSelectedRegionId(null); }} disabled={currentPageIndex === 0} className="p-1 hover:bg-slate-700 rounded shadow-sm disabled:opacity-30 text-white"><ChevronLeft className="w-4 h-4" /></button>
                                <span className="text-xs font-mono min-w-[60px] text-center text-slate-300">Page {currentPageIndex + 1} / {pages.length}</span>
                                <button onClick={() => { setCurrentPageIndex(p => Math.min(pages.length - 1, p + 1)); setSelectedRegionId(null); }} disabled={currentPageIndex === pages.length - 1} className="p-1 hover:bg-slate-700 rounded shadow-sm disabled:opacity-30 text-white"><ChevronRight className="w-4 h-4" /></button>
                            </div>
                        )}
                    </div>
                    <div className="flex items-center gap-2">
                        <button onClick={() => setScale(s => Math.max(0.2, s - 0.1))} className="p-1.5 hover:bg-slate-800 rounded text-slate-400 hover:text-white transition-colors"><ZoomOut className="w-5 h-5" /></button>
                        <span className="text-xs font-mono w-12 text-center text-slate-400">{Math.round(scale * 100)}%</span>
                        <button onClick={() => setScale(s => Math.min(3, s + 0.1))} className="p-1.5 hover:bg-slate-800 rounded text-slate-400 hover:text-white transition-colors"><ZoomIn className="w-5 h-5" /></button>
                    </div>
                </div>
                ...
                {showResults && currentPage && currentPage.results && (
                    <div className="absolute bottom-0 left-0 right-0 bg-slate-900 border-t border-slate-700 shadow-xl max-h-[300px] flex flex-col transition-transform z-30">
                        <div className="px-4 py-2 border-b border-slate-800 flex justify-between items-center bg-slate-900">
                            ...
                            <button onClick={() => setShowResults(false)} className="text-slate-400 hover:text-white"><ChevronDown /></button>
                        </div>
