...
import { Upload, FileSpreadsheet, Plus, Trash2, CheckCircle2, ScanSearch, Settings, ChevronRight, ChevronDown, ChevronUp, Download, ZoomIn, ZoomOut, LayoutTemplate, ChevronLeft, Layers, ScanLine, GraduationCap, Hash, UserSquare2, X, Loader2, RotateCw, Menu } from 'lucide-react';
...
const App = () => {
  ...
  useEffect(() => {
    // Tokenize by whitespace/newlines/punctuation
    // Logic: Only accept tokens that contain ONLY A, B, C, D. Reject "Answer", "1.", etc.
    const rawText = answerKeyInput.toUpperCase();
    const tokens = rawText.split(/[^A-D]+/); // Split by anything that isn't A-D
    
    // Actually, splitting by non-A-D is not quite right because "Answer" would split into "A" and "er".
    // Better approach: Split by whitespace, then check each word.
    ...
    words.forEach(word => {
        // Remove non-alphabetic chars like '.' or ')' if attached? 
        // User requirement: "if a word only contain ABCD, it's accepted, but if not like 'Answer' then the whole word is rejected"
        // Let's strip punctuation first? Or check strict match?
        // Strict match `^[A-D]+$` seems safest based on request.
        
        // Clean word of common punctuation to be safe? "1.A" -> "1A" (Reject) or "1." "A" (Accept A).
        // Let's rely on user pasting space separated.
        
        // Check if word consists ONLY of A, B, C, D
        if (/^[A-D]+$/.test(word)) {
            // It is a valid sequence. Map chars to questions.
  ...
  useEffect(() => {
    ...
    img.onload = () => {
        ...
        regionsToDraw.forEach(region => {
            ctx.strokeStyle = region.id === selectedRegionId ? '#60a5fa' : '#ef4444'; // Blue-400 : Red-500
            ctx.lineWidth = 2;
            ...
            rowLayouts.forEach((row, idx) => {
                ...
                const rowH = row.h;
                if (row.isGap) {
                    ctx.fillStyle = 'rgba(71, 85, 105, 0.3)'; // Slate-600 with opacity
                ...
                pageRegions.forEach(region => {
                    ...
                    if (region.type === 'id') {
                        ...
                        if (candidates.length === 0) {
                            ...
                            const ratio = winner.fillRatio / (runnerUp.fillRatio + 0.0001);

                            if (diff > 0.15 || ratio > 2.0) {
                                label = winner.label;
  ...
  const exportExcel = () => {
    ...
    const fullMarkRef = refTotalMarks; // Using formula from Sheet 1 marks sum

    // For simplicity, we create a small data table for the stats sheet
    // We can't do complex variable refs easily, but we can do fixed ranges
    
    const statsData = [
        ["Metric", "Value"],
        ["Full Mark", { t:'n', f: fullMarkRef }], // Calculated dynamically
        ["Max", { t:'n', f: `MAX(${scoreRange})` }],
        ["Min", { t:'n', f: `MIN(${scoreRange})` }],
        ["Mean", { t:'n', f: `AVERAGE(${scoreRange})`, z:'0.00' }],
        ["Median", { t:'n', f: `MEDIAN(${scoreRange})` }],
        ["SD", { t:'n', f: `STDEV.P(${scoreRange})`, z:'0.00' }],
        ...
        ["No. of Passes", { t:'n', f: `COUNTIF(${scoreRange},">="&B11)` }],
  ...
  const renderWeightSettings = () => (
      <div className="space-y-2">
         <button onClick={() => setIsMarksSettingsOpen(!isMarksSettingsOpen)} className="w-full flex items-center justify-between text-xs font-semibold uppercase text-slate-500 tracking-wider hover:text-slate-300 bg-slate-800 p-2 rounded border border-slate-700">
             ...
             <div className="bg-slate-800 p-2 rounded-lg border border-slate-700 space-y-2">
                 {weightSections.map((section, idx) => (
                     <div key={section.id} className="flex items-center gap-1 text-xs">
                         <input type="number" min="1" className="w-12 p-1 border border-slate-600 rounded text-center bg-slate-700 text-white" value={section.start} onChange={(e) => {
                             const valStr = e.target.value;
                             if (valStr === '') { const newSecs = [...weightSections]; newSecs[idx].start = ''; setWeightSections(newSecs); return; }
                             let val = parseInt(valStr); if (val < 1) val = 1; const newSecs = [...weightSections]; newSecs[idx].start = val; setWeightSections(newSecs);
                         }} placeholder="Start" />
                         <span>-</span>
                         <input type="number" min="1" className="w-12 p-1 border border-slate-600 rounded text-center bg-slate-700 text-white" value={section.end} 
                         onChange={(e) => {
                             const valStr = e.target.value;
                             if (valStr === '') { const newSecs = [...weightSections]; newSecs[idx].end = ''; setWeightSections(newSecs); return; }
                             let val = parseInt(valStr); if (val < 1) val = 1; const newSecs = [...weightSections]; newSecs[idx].end = val; setWeightSections(newSecs);
                         }} 
                         onBlur={() => {
                             const newSecs = [...weightSections];
                             const s = newSecs[idx].start; const e = newSecs[idx].end;
                             if (s !== '' && e !== '' && e < s) { newSecs[idx].end = s; setWeightSections(newSecs); }
                         }} placeholder="End" />
                         <span>:</span>
                         <input type="number" min="0.1" step="0.5" className="w-10 p-1 border border-slate-600 rounded text-center bg-slate-700 text-white" value={section.mark} onChange={(e) => {
                             const valStr = e.target.value;
                             if (valStr === '') { const newSecs = [...weightSections]; newSecs[idx].mark = ''; setWeightSections(newSecs); return; }
                             let val = parseFloat(valStr); if (val <= 0) val = 1; if (isNaN(val)) val = 1; const newSecs = [...weightSections]; newSecs[idx].mark = val; setWeightSections(newSecs);
                         }} placeholder="Mark" />
                 ...
                 }} className="w-full py-1 text-xs bg-blue-900/30 text-blue-400 rounded border border-blue-800 hover:bg-blue-900/50">+ Add Range</button>
             </div>
         )}
      </div>
  );
  ...
  return (
    <div className="flex flex-col md:flex-row h-screen bg-slate-950 text-slate-100 font-sans overflow-hidden">
      ...
      {/* MOBILE HEADER */}
      <div className="md:hidden bg-slate-900 border-b border-slate-700 p-4 flex justify-between items-center z-20">
      ...
      {/* SIDEBAR */}
      <div className={`${sidebarOpen ? 'flex' : 'hidden'} md:flex w-full md:w-80 bg-slate-900 border-r border-slate-700 flex-col shadow-xl z-10 absolute md:relative h-full`}>
        <div className="hidden md:block p-4 border-b border-slate-700 bg-slate-900 text-white">
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-6">
          ...
          <div className="space-y-2">
            ...
            <textarea 
                placeholder="Paste answers (e.g. '1.A 2.B' or 'ABCDE')" 
                className="w-full h-24 p-2 text-sm border border-slate-600 rounded bg-slate-800 text-slate-100 focus:bg-slate-700 transition-colors outline-none font-mono placeholder-slate-500" 
          ...
          <div className="space-y-2">
            <button onClick={() => setIsTemplateSettingsOpen(!isTemplateSettingsOpen)} className="w-full flex items-center justify-between text-xs font-semibold uppercase text-slate-500 tracking-wider hover:text-slate-300 bg-slate-800 p-2 rounded border border-slate-700">
            ...
            {isTemplateSettingsOpen && (
                ...
                <div className="p-4 bg-blue-900/20 text-blue-300 text-sm rounded-lg border border-blue-800">
                    ...
                    <p className="text-xs mb-2 text-slate-400">Drag <strong>red boxes</strong> to align.</p>
                    <button onClick={resetTemplate} className="w-full py-2 mt-3 text-xs font-medium text-white bg-blue-600 rounded-lg hover:bg-blue-700 transition-colors flex justify-center items-center gap-2">
                ...
                {currentRegions.length > 0 && (
                    <div className="space-y-2">
                        ...
                        {currentRegions.sort((a, b) => a.y - b.y).map((r, i) => (
                            <button key={r.id} onClick={() => setSelectedRegionId(r.id)} className={`w-full text-left px-3 py-2 rounded text-sm flex justify-between items-center ${selectedRegionId === r.id ? 'bg-blue-900/50 text-blue-300 border border-blue-700' : 'bg-slate-800 border border-slate-700 hover:bg-slate-700'}`}>
      ...
      <div className="flex-1 flex flex-col bg-slate-950 relative">
        <div className="h-14 bg-slate-900 border-b border-slate-700 flex items-center px-4 justify-between">
           <div className="flex items-center gap-4 text-sm text-slate-400 overflow-hidden">
               {file ? <span className="font-semibold text-white truncate max-w-[150px] md:max-w-[300px]">{file.name}</span> : <span>No file loaded</span>}
               {pages.length > 1 && (
                   <div className="flex items-center bg-slate-800 rounded-lg p-1 gap-2 ml-4">
                       ...
                       <span className="text-xs font-mono min-w-[60px] text-center text-white">Page {currentPageIndex + 1} / {pages.length}</span>
           ...
           <div className="flex items-center gap-2">
               <button onClick={() => setScale(s => Math.max(0.2, s - 0.1))} className="p-1.5 hover:bg-slate-800 rounded text-slate-300 bg-slate-900"><ZoomOut className="w-5 h-5"/></button>
               <span className="text-xs font-mono w-12 text-center text-slate-300">{Math.round(scale * 100)}%</span>
               <button onClick={() => setScale(s => Math.min(3, s + 0.1))} className="p-1.5 hover:bg-slate-800 rounded text-slate-300 bg-slate-900"><ZoomIn className="w-5 h-5"/></button>
           </div>
        </div>
